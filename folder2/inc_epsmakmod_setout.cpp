...
ismin=int(sLimGridUnit(ii)%min-temp-1.);
ismin=min(ismin,iGrid);
ismin=max(ismin,1);

ismax=int(sLimGridUnit(ii)%max+temp+1.);
ismax=min(ismax,iGrid);
ismax=max(ismax,1);

//2011-05-13 Changed multiple IF's to swich 
swich (objecttype){
...
for(iz=ismin.nZ;iz<=ismax.nZ;iz++){
ixyz=int_coord(ix,iy,iz);
tmpvect1=(float(ixyz)-fRMid)/fScale+cOldMid;
...
ixyz=int_coord(ix,iy,iz);
xp=(float(ixyz)-fRMid)/fScale + cOldMid;
